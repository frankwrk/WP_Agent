    ...
    public static function handle(\WP_REST_Request $request)
    {
        ...
        if ($backendBaseUrl === '') {
            return new \WP_Error(
                ...
                'WP_AGENT_BACKEND_BASE_URL is not configured',
                ['status' => 500]
            );
        }

        $pairUrl = $backendBaseUrl . '/api/v1/installations/pair';
        ...
        $backendBaseUrlFromBackend = (string) (($decoded['data']['backend_base_url'] ?? ''));
        $backendBaseUrlToStore = $backendBaseUrlFromBackend !== ''
            ? rtrim($backendBaseUrlFromBackend, '/')
            : $backendBaseUrl;
